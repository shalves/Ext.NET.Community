...
namespace Ext.Net
{
    ...
    public partial class DatePicker : Component, IDate, IAutoPostBack, IXPostBackDataHandler, IPostBackEventHandler, IField
    {
        ...
        public object SelectedValue
        {
            ...
            set
            {
                ...
                else
                {
                    try
                    {
                        this.SelectedDate = DateTime.Parse(init.ToString(), this.SafeResourceManager != null ? this.ResourceManager.CurrentLocale.DateTimeFormat : CultureInfo.InvariantCulture.DateTimeFormat);
                    }
        ...
        [Category("4. DatePicker")]
        [DefaultValue("")]
        [DirectEventUpdate(MethodName = "SetDisabledDatesRE")]
        ...
        protected virtual bool LoadPostData(string postDataKey, NameValueCollection postCollection)
        {
            ...
            if (val.IsNotEmpty())
            {
                ...
                {
                    this.SuspendScripting();
                    this.SelectedDate = DateTime.ParseExact(val, "yyyy\\-MM\\-dd\\THH\\:mm\\:ss", this.HasResourceManager ? this.ResourceManager.CurrentLocale : CultureInfo.InvariantCulture);
                }
                catch
                {
                    this.SelectedDate = (DateTime)this.EmptyValue;
        ...
        /*  Public Methods
            -----------------------------------------------------------------------------------------------*/
        /// <summary>
        /// Sets a JavaScript regular expression used to disable a pattern of dates (defaults to null).
        /// </summary>
        /// <param name="disabledDatesRE"></param>
        [Description("Sets a JavaScript regular expression used to disable a pattern of dates (defaults to null).")]
        public void SetDisabledDatesRE(string disabledDatesRE)
        {
            this.Call("setDisabledDates", new JRawValue(disabledDatesRE.FormatRegexPattern()));
        }
